...
After deployment completes, Cloud Run will print a **Service URL** like:
```
https://biasauditor-api-xxxxxxxxxx-uc.a.run.app
```
...
## Step 7 — Update firebase.json with your Cloud Run service

Open `firebase.json`. The rewrites already reference `serviceId: "biasauditor-api"` and `region: "us-central1"`. As long as you named your Cloud Run service `biasauditor-api` in the same region, no changes are needed.

If you used a different service name or region, update the two `run` blocks in `firebase.json` to match.
...
Firebase will print your live Hosting URL:
```
...
```bash
# Re-build and push backend image
docker build -t gcr.io/YOUR_PROJECT_ID/biasauditor-api . && \
docker push gcr.io/YOUR_PROJECT_ID/biasauditor-api && \
